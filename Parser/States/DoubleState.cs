...
public class DoubleState : IState
{
    ...
    public DoubleState(List<ParserError> errors, StringHelper stringHelper, Dictionary<LexemeType, IState> StateMap)
    {
        this.errors = errors;
    ...
    public bool Handle()
    {
        ...
        foreach (char c in "double ")
        {
            ParserError error = new ParserError("Ожидалось ключевое слово \"double\"", stringHelper.Index + 1, stringHelper.Index + 1);
            while (true)
            {
                if (!stringHelper.CanGetNext)
                {
