...
public class ConstState : IState
{
    ...
    public ConstState(List<ParserError> errors, StringHelper stringHelper, Dictionary<LexemeType, IState> StateMap)
    {
        this.errors = errors;
    ...
    public bool Handle()
    {
        ...
        foreach (char c in "const ")
        {
            ParserError error = new ParserError("Ожидалось ключевое слово \"const\"", stringHelper.Index + 1, stringHelper.Index + 1);
            while (true)
            {
        ...
        StateMap[LexemeType.DOUBLE].Handle();
        return true;
    }
}
